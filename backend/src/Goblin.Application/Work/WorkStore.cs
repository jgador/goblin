...
public sealed class WorkStore(GoblinDbContext db, IDbContextOutbox<GoblinDbContext> outbox)
{
    ...
    public async Task<WorkView> GetAsync(Guid id, CancellationToken token = default) =>
        View(await db.WorkItems.AsNoTracking().SingleOrDefaultAsync(x => x.Id == id, token)
            ?? throw new ApplicationFailure("work_not_found"));

    public Task<AgentView[]> AgentsAsync(CancellationToken token = default) => db.Agents.AsNoTracking()
        .OrderBy(x => x.Name).Select(x => new AgentView(x.Id, x.Name, x.ConnectionId, x.Model)).ToArrayAsync(token);

    public Task<ConnectionView[]> ConnectionsAsync(CancellationToken token = default) => db.Connections.AsNoTracking()
    ...
    public Task<AttemptRow[]> RecoverableAsync(CancellationToken token = default) => db.ExecutionAttempts.AsNoTracking()
