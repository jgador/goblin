...
spec = importlib.util.spec_from_file_location("generate_protocol", Path(__file__).with_name("generate-protocol.py"))
...
class ProtocolNamingTests(unittest.TestCase):
    ...
    def test_repeated_payload_name_is_shortened_without_changing_wire_names(self):
        ...
        files = self.generate({owner: {
            "type": "object",
            "properties": {"acceptWithExecpolicyAmendment": {
                "type": "object",
        ...
        self.assertEqual({owner + ".g.cs", "AcceptWithExecpolicyAmendmentDetails.g.cs"}, set(files))
        self.assertIn('[JsonPropertyName("acceptWithExecpolicyAmendment")]', files[owner + ".g.cs"])
        self.assertIn('[JsonPropertyName("execpolicy_amendment")]', files["AcceptWithExecpolicyAmendmentDetails.g.cs"])

    def test_generic_property_names_keep_their_owner_context(self):
        self.assertEqual("GranularAskForApprovalGranular", protocol.nested_type_name(
    ...
    def test_wrappers_do_not_take_their_payload_or_base_name(self):
        ...
        self.assertEqual(payload + "Variant", protocol.variant_type_name(
            payload, "McpElicitationSingleSelectEnumSchema", wrapped=True))
        self.assertEqual("ResultVariant", protocol.variant_type_name("Result", "Result"))

    def test_collisions_preserve_named_types_and_are_independent_of_definition_order(self):
        named = {"type": "object", "properties": {"original": {"type": "boolean"}}}
        definitions = {"AcceptWithPolicyDetails": named}
        for owner, value_type in [("FirstAcceptWithPolicyDecision", "string"),
                                  ("SecondAcceptWithPolicyDecision", "integer")]:
            definitions[owner] = {
                "type": "object",
                "properties": {"acceptWithPolicy": {
                    "type": "object", "properties": {"value": {"type": value_type}},
                }},
            }
        files = self.generate(definitions)
        self.assertEqual(files, self.generate(dict(reversed(list(definitions.items())))))
        self.assertIn('public bool? Original', files["AcceptWithPolicyDetails.g.cs"])
        self.assertIn("AcceptWithPolicyDetails2? AcceptWithPolicy", files["FirstAcceptWithPolicyDecision.g.cs"])
        self.assertIn("AcceptWithPolicyDetails3? AcceptWithPolicy", files["SecondAcceptWithPolicyDecision.g.cs"])

    def test_identical_variants_with_different_bases_stay_distinct(self):
        variant = {"title": "SharedVariant", "type": "object",
                   "properties": {"value": {"type": "string"}}, "required": ["value"]}
        files = self.generate({"FirstResult": {"oneOf": [variant]}, "SecondResult": {"oneOf": [variant]}})
        self.assertIn("class SharedVariant : FirstResult", files["SharedVariant.g.cs"])
        self.assertIn("class SharedVariant2 : SecondResult", files["SharedVariant2.g.cs"])
