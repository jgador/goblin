...
public sealed class ApiKeyVerifierTests
{
    ...
    public async Task ChecksModelsEndpointAndClassifiesResponses(int status, string expected)
    {
        ...
        {
            IntegrationFailure error = await Assert.ThrowsAsync<IntegrationFailure>(() => verifier.VerifyAsync(Key));
    ...
    public async Task NetworkErrorsNeverExposeCredentials()
    {
        ...
        IntegrationFailure error = await Assert.ThrowsAsync<IntegrationFailure>(() => new ApiKeyVerifier(client).VerifyAsync(Key));
