...
-- Goblin-owned table IDs and their references use bigint uniformly. Runtime
-- sessions are opaque references; Wolverine's vendor-owned keys remain below.
...
-- A catalog belongs to one account generation and one installed Codex CLI
-- revision. A failed refresh preserves the last successful catalog.
CREATE TABLE public.connection_model_catalogs (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    connection_id bigint NOT NULL UNIQUE REFERENCES public.connections(id),
    auth_generation bigint NOT NULL,
    executable_stamp text NOT NULL,
    catalog jsonb,
...
ALTER TABLE public.execution_attempts ADD COLUMN turn_number integer NOT NULL DEFAULT 1 CHECK (turn_number > 0);
ALTER TABLE public.execution_attempts ADD COLUMN workspace_retained boolean NOT NULL DEFAULT false;
DROP INDEX public.one_execution_per_connection;
CREATE UNIQUE INDEX one_execution_per_connection ON public.execution_attempts(connection_id)
WHERE status IN ('Starting', 'Running', 'CancellationRequested', 'Uncertain') OR cleanup_pending OR workspace_retained;

CREATE TABLE public.workspace_checkpoints (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    work_id bigint NOT NULL REFERENCES public.work_items(id),
    attempt_id bigint NOT NULL REFERENCES public.execution_attempts(id),
...
-- Successful setup observations are scoped to a connected repository. Keep
-- branch/configuration variants and execution provenance rather than a single
-- mutable recipe. The checkpoint fences the observation to a saved turn.
CREATE TABLE public.repository_setup_memories (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    repository_id bigint NOT NULL REFERENCES public.github_repositories(id),
    github_connection_id bigint NOT NULL REFERENCES public.github_connections(id),
    account_id text NOT NULL,
    checkpoint_id bigint NOT NULL REFERENCES public.workspace_checkpoints(id),
    work_id bigint NOT NULL REFERENCES public.work_items(id),
    attempt_id bigint NOT NULL REFERENCES public.execution_attempts(id),
    turn_number integer NOT NULL CHECK (turn_number > 0),
    topic text NOT NULL CHECK (length(topic) BETWEEN 1 AND 80),
    environment text NOT NULL CHECK (length(environment) BETWEEN 1 AND 1000),
    fingerprint text NOT NULL CHECK (length(fingerprint) = 64),
    observation jsonb NOT NULL CHECK (octet_length(observation::text) <= 65536),
    verified_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (attempt_id, turn_number, topic)
);
CREATE INDEX repository_setup_recall ON public.repository_setup_memories
    (github_connection_id, account_id, repository_id, verified_at DESC);

CREATE TABLE public.workspace_sessions (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    work_id bigint NOT NULL REFERENCES public.work_items(id),
    attempt_id bigint NOT NULL REFERENCES public.execution_attempts(id),
    checkpoint_id bigint REFERENCES public.workspace_checkpoints(id),
...
CREATE UNIQUE INDEX one_inspection_per_work ON public.workspace_sessions(work_id)
WHERE state IN ('Queued', 'Starting', 'Available', 'Stopping');
