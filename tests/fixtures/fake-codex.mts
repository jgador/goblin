...
const result = (id: RequestId | undefined, value: unknown) =>
...
for await (const line of createInterface({ input: process.stdin })) {
    ...
    if (method.startsWith("thread/") || method.startsWith("turn/")) {
        await appendFile(
            join(root, "prompt-requests.jsonl"),
    ...
    if (method === "initialize") {
        ...
        if (scenario === "reordered") {
            ...
            if (reordered.length === 2) {
                for (const requestId of reordered.reverse())
                    result(requestId, {
                        account: {
                            type: "chatgpt",
                            email: String(requestId),
                            planType: "plus",
                        },
        ...
        try {
            account = JSON.parse(
                await readFile(accountFile, "utf8"),
            ) as Account;
        } catch {}
        ...
        if (scenario === "fail-device") {
            send({
                ...
                error: {
                    code: 400,
                    message: "Upstream secret sk-THIS-MUST-NOT-LEAK",
                },
            });
            continue;
        }
        ...
        await writeFile(
            accountFile,
            JSON.stringify({ type: "apiKey", apiKey: params.apiKey }),
            { mode: 0o600 },
        );
        ...
        const { threadId, input } = params as {
            threadId: string;
            input: { text: string }[];
        };
        ...
        function complete() {
            ...
            const message = {
                ...
                phase: "final_answer",
                text:
                    scenario === "prompt-large"
                        ? "x".repeat(8001)
                        : scenario === "work-result"
                          ? JSON.stringify({
                                kind: "result",
                                text: "A proposed Work outcome",
                            })
                          : scenario === "work-input"
                            ? JSON.stringify({
                                  kind: "input",
            ...
            send({
                ...
                params: {
                    ...
                    turnId,
                    item: { ...message, text: "Wrong thread" },
                },
            });
            send({
                method: "item/completed",
            ...
            send({
                ...
                params: {
                    ...
                    turn: {
                        ...
                        error,
                        items: scenario === "prompt-empty" ? [] : [message],
                    },
                },
            });
        }
