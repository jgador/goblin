...
async function pollAccount(page: Page) {
    ...
    await page.waitForFunction(
        () =>
            document.getElementById("card")?.getAttribute("aria-busy") ===
            "false",
    );
}

test("saved accounts check automatically and require a manual retry after failures", async ({
    page,
}) => {
            ...
            failure
                ? {
                      ...
                      json: {
                          error: {
                              code: failure.code,
                              message: "Internal prompt details",
                          },
                      },
                  }
                : { json: reply },
    ...
    for (const [index, failure] of failures.entries()) {
        ...
        await expect(page.locator("#connection-description")).toContainText(
            failure.detail,
        );
        await expect(
...
test("network errors fail the check and a retry can recover", async ({
    page,
}) => {
    await savedAccount(page);
    let checks = 0;
...
test("an expired workspace session clears the check and verifies again after unlocking", async ({
    page,
}) => {
    await savedAccount(page);
            ...
            await route.fulfill({
                ...
                json: {
                    error: {
                        ...
                        message: "Unlock the workspace to continue.",
                    },
                },
            });
...
test("account changes clear a previous success and trigger a fresh check", async ({
    page,
}) => {
    await page.clock.install();
...
test("losing account status removes a previous success without automatically sending another check", async ({
    page,
}) => {
    await page.clock.install();
    await savedAccount(page);
