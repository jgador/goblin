...
async function start(t: TestContext, options: Partial<BackendOptions> = {}) {
    const dataDir =
        options.dataDir ??
        (await mkdtemp(join(tmpdir(), "goblin-local-login-")));
    t.after(() => rm(dataDir, { recursive: true, force: true }));
    const publicOrigin = options.publicOrigin ?? "http://localhost:8787";
    const passwordHashFile =
        options.passwordHashFile ?? join(dataDir, "owner-password");
...
test("local provisioning uses Azure's verifier and requires the chosen password on every loopback origin", async (t) => {
    for (const host of ["localhost", "127.0.0.1", "[::1]"]) {
        await t.test(host, async (t) => {
            ...
            assert.equal(
                (await request("/api/session", localPassword.trim())).status,
                401,
            );
            assert.equal(
                (
                    await request("/api/session", localPassword, "", {
            ...
            assert.equal(
                (
                    await request("/api/session", undefined, "", {
            ...
            assert.equal(
                (
                    await (
                        await request("/api/session", undefined, cookie)
                    ).json()
                ).authenticated,
                true,
            );
            assert.equal(
                (await request("/api/status", undefined, cookie)).status,
                200,
            );
            assert.equal(
                (await request("/api/session/lock", "", cookie)).status,
                200,
            );
            assert.equal(
                (await request("/api/status", undefined, cookie)).status,
...
test("a configured local password survives restart and replacing it invalidates previous credentials and sessions", async (t) => {
    ...
    assert.equal(
        (await restarted.request("/api/status", undefined, cookie)).status,
        401,
    );
    assert.equal(
        (await restarted.request("/api/session", localPassword)).status,
        200,
    );
    ...
    assert.deepEqual(
        await (await locked.request("/api/session", undefined, cookie)).json(),
        { authenticated: false },
    );
    assert.equal(
        (await locked.request("/api/status", undefined, cookie)).status,
        401,
    );
    assert.equal((await locked.request("/api/session", "goblin")).status, 401);
    assert.equal(
        (await locked.request("/api/session", localPassword)).status,
        401,
    );
    ...
    for (const options of [
        ...
        await t.test(JSON.stringify(options), async (t) => {
            const dataDir = await mkdtemp(
                join(tmpdir(), "goblin-required-password-"),
            );
            t.after(() => rm(dataDir, { recursive: true, force: true }));
            await assert.rejects(
                startBackend({ dataDir, ...options }),
                /Set GOBLIN_PASSWORD_HASH_FILE/,
            );
