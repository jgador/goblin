...
test("local source snapshots include edits and exclude ignored local installation data and credentials", async (t) => {
    ...
    await writeFile(
        join(repo, ".goblin-local/login-password"),
        "test-only ignored credential",
    );
...
test("building a local bundle does not overwrite generated Azure template inputs", async (t) => {
    ...
    await cp(
        ...
        join(branding, "icon-light.svg"),
    );
    await cp(
        "deploy/azure/build-setup-bundle.py",
        join(azure, "build-setup-bundle.py"),
    );
    ...
    for (const name of ["setup-bundle.b64", "setup-bundle.sha256"])
        await writeFile(join(azure, name), "existing generated input\n");
    execFileSync("python3", [
        join(azure, "build-setup-bundle.py"),
        "--output-only",
        "--output",
    ...
    for (const name of ["setup-bundle.b64", "setup-bundle.sha256"])
        assert.equal(
            await readFile(join(azure, name), "utf8"),
