...
async function fixture() {
  ...
  await mkdir(bin);
  await cp("deploy/postgres", join(root, "deploy/postgres"), { recursive: true });
  const web = join(root, "backend/src/Goblin.Web");
  await mkdir(web, { recursive: true });
  await writeFile(join(web, "appsettings.json"), JSON.stringify({
    ConnectionStrings: { Goblin: "Host=old-database;Username=goblin_app;Password=old-test-value" },
...
`, { mode: 0o700 });
  const run = (environment: Record<string, string | undefined> = {}) => execFileSync("bash", [join(root, "deploy/postgres/setup.sh"), "--port", "55432"], {
    env: { ...process.env, PATH: `${bin}:${process.env.PATH}`, GOBLIN_POSTGRES_SETUP_TEST: root, ...environment },
    encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: 15_000,
  });
  const calls = async (): Promise<string[][]> => (await readFile(join(root, "calls.jsonl"), "utf8")).trim().split("\n").map(line => JSON.parse(line));
...
test("certificate setup removes passwords, protects exported keys, and preserves identities/data on rerun", async t => {
  ...
  const toolPath = join(root, "backend/tools/Goblin.Database/appsettings.json");
  ...
  assert.match(web.ConnectionStrings.Goblin, /Host=goblin-postgres;Port=5432;.*Username=goblin_app;SSL Mode=VerifyFull;/);
  assert.match(web.ConnectionStrings.Goblin, /SSL Key="\/etc\/goblin-postgres\/tls.key"/);
  assert.match(tooling.ConnectionStrings.GoblinAdmin, /Host=localhost;Port=55432;.*Username=goblin_admin;SSL Mode=VerifyFull;/);
  assert.ok(tooling.ConnectionStrings.Goblin.includes(root.replaceAll('"', '""')));
  assert.doesNotMatch(webText + toolText, /Password=|Trust Server Certificate|private-key/);
  for (const role of ["app", "admin"]) {
    const directory = join(root, ".goblin-postgres", role);
    assert.equal((await stat(directory)).mode & 0o777, 0o700);
    assert.equal((await stat(join(directory, "tls.key"))).mode & 0o777, 0o600);
    assert.equal(await readFile(join(directory, "tls.key"), "utf8"), role + "-private-key");
  }
  assert.equal((await stat(toolPath)).mode & 0o777, 0o600);
  const adminSecret = await readFile(join(root, "goblin-postgres-admin"), "utf8");
  const ca = await readFile(join(root, "goblin-postgres-ca"), "utf8");
  const second = run();
  assert.equal(await readFile(join(root, "goblin-postgres-admin"), "utf8"), adminSecret);
  ...
  assert.doesNotMatch(output, /app-private-key|admin-private-key|CA-private-key/);
  assert.equal((await calls()).filter(args => args[0] === "create" && args[2] === "-").length, 1);
});

test("setup fails before changing PostgreSQL when certificate issuance or API access fails", async t => {
  for (const environment of [{ GOBLIN_POSTGRES_SETUP_DENY: "true" }, { GOBLIN_POSTGRES_CERT_FAILED: "true" }]) {
    const { root, run, calls } = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    const before = await readFile(join(root, "backend/src/Goblin.Web/appsettings.json"), "utf8");
    assert.throws(() => run(environment));
    assert.ok(!(await calls()).some(args => args[0] === "apply" && args[1] === "-k"));
    assert.equal(await readFile(join(root, "backend/src/Goblin.Web/appsettings.json"), "utf8"), before);
  }
});
...
test("setup refuses to replace a lost CA or initialization Secret for existing PostgreSQL data", async t => {
  for (const missing of ["goblin-postgres-ca", "goblin-postgres-admin"]) {
    ...
    assert.ok(!(await calls()).some(args => args[0] === "apply" && args[2] !== "-"));
    assert.ok(!(await calls()).some(args => args[0] === "create" && args[1] === "-f"));
  }
});
...
test("incomplete exported client credentials do not overwrite settings or keys", async t => {
  ...
  assert.throws(() => execFileSync("python3", [join(root, "deploy/postgres/write-appsettings.py")], {
    input: JSON.stringify({ items: [] }), stdio: ["pipe", "pipe", "pipe"],
  }));
  assert.equal(await readFile(file, "utf8"), before);
  assert.equal(await readFile(join(root, ".goblin-postgres/app/tls.key"), "utf8"), "app-private-key");
});
...
test("connecting an existing Sandbox preserves its image, origin, storage and other containers", async t => {
  ...
  const original = { containers: [
    ...
    { name: "auth", image: "existing-image", env: [{ name: "GOBLIN_PUBLIC_ORIGIN", value: "http://existing.test" }],
      volumeMounts: [{ name: "data", mountPath: "/data" }] },
  ], volumes: [{ name: "data", persistentVolumeClaim: { claimName: "keep-me" } }] };
  const patch = (spec: object) => execFileSync("python3", [join(root, "deploy/postgres/configure-app.py")], {
    input: JSON.stringify({ spec: { podTemplate: { spec } } }), encoding: "utf8",
  });
  ...
  assert.deepEqual(updated.containers[1].env[0], original.containers[1]!.env![0]);
  assert.deepEqual(updated.volumes[0], original.volumes[0]);
  assert.equal(updated.volumes[1].secret.secretName, "goblin-postgres-app-tls");
