...
test("the standalone UI serves only status and assets, with no mutation or file access", async (t) => {
  ...
  t.after(() => setup.close());
  await writeFile(join(setup.root, "owner-password"), "private-test-verifier");
  for (const path of ["/owner-password", "/setup/../owner-password", "/setup/status/../../owner-password", "/logs", "/readyz", "/setup/retry"]) {
    const response = await fetch(setup.url + path);
    assert.equal(response.status, 404);
    assert.ok(!(await response.text()).includes("private-test-verifier"));
  }
  assert.equal((await fetch(setup.url + "/setup/status", { method: "POST" })).status, 501);
  ...
  assert.equal(status.steps.find((step: { id: string }) => step.id === "k3s").status, "failed");
  setup.transition("begin");
  setup.transition("start", "k3s");
  const retried = JSON.parse(await readFile(setup.path, "utf8"));
  assert.equal(retried.attempt, 2);
  assert.equal(retried.steps.find((step: { id: string }) => step.id === "k3s").attempt, 2);
...
test("local setup health cannot report ready when the public port is occupied", async (t) => {
  ...
  assert.throws(() => execFileSync("python3", [join(setup.root, "goblin-setup.pyz"), "serve",
    "--state", setup.path, "--host", "127.0.0.1", "--port", new URL(setup.url).port,
    "--health-socket", socket], { stdio: ["ignore", "pipe", "pipe"] }), /Address already in use/);
