...
if (!["scan", "staged", "history"].includes(mode) || process.argv.length > 3) {
    console.error(
        "Usage: node scripts/check-secrets.mts [scan|staged|history]",
    );
    process.exit(2);
}
...
function indexEntries(): IndexEntry[] {
    return paths(["ls-files", "--stage", "-z"]).map((entry) => {
        ...
        if (stage !== "0")
            throw new Error("Resolve merge conflicts before scanning.");
        if (!["100644", "100755", "120000"].includes(fileMode)) {
            throw new Error(
                `Cannot scan Git entry ${JSON.stringify(path)} (mode ${fileMode}); scan submodules separately.`,
            );
        }
        return { object, path };
    });
...
function snapshotIndex(entries: IndexEntry[], directory: string) {
    ...
    for (const { object, path } of entries) {
        ...
        const [actual, type, length] = blobs
            .subarray(offset, end)
            .toString("ascii")
            .split(" ");
        ...
        if (
            ...
            throw new Error(
                "Unable to read the complete Git index; scan incomplete.",
            );
        }
        offset = end + 1;
        if (offset + size >= blobs.length)
            throw new Error("Truncated index snapshot; scan incomplete.");
...
function snapshotWorktree(directory: string) {
    const inventory = new Set(
        paths(["ls-files", "--cached", "--others", "--exclude-standard", "-z"]),
    );
    let count = 0;
    for (const path of inventory) {
        ...
        if (realpathSync(dirname(source)) !== dirname(source)) {
            throw new Error(
                `Cannot scan ${JSON.stringify(path)} through a symlink directory; review it separately.`,
            );
        }
        if (stat.isSymbolicLink()) {
            ...
            throw new Error(
                `Cannot scan ${JSON.stringify(path)}; expected a file or symlink.`,
            );
        }
        count++;
    }
...
function scan(
    ...
    if (
        ...
        throw new Error(
            "Gitleaks failed; scan incomplete. Check the installed version and .gitleaks.toml. Raw output is withheld to protect secrets.",
        );
    }
    ...
    for (const finding of findings) {
        ...
        const commit = finding.Commit
            ...
            : "";
        console.error(
            `[${label}${commit}] ${JSON.stringify(path)}:${finding.StartLine} (${finding.RuleID})`,
        );
    }
    if (result.status === 1 && findings.length === 0) {
        throw new Error(
            "Gitleaks failed without a finding report; scan incomplete.",
        );
    }
    console.log(
        `${label}: ${findings.length} finding(s)${count === undefined ? "" : ` in ${count} file(s)`}.`,
    );
...
try {
    ...
    if (
        ...
        numbers[0] !== 8 ||
        numbers[1] < 30 ||
        (numbers[1] === 30 && numbers[2] < 1)
    ) {
        throw new Error(
            "Install Gitleaks 8.30.1 or a newer 8.x release on PATH; see docs/secret-scanning.md.",
        );
    }
    ...
    if (found) {
        console.error(
            "Possible secrets found. Review the locations above, remove sensitive values, and restage any corrected files before committing.",
        );
        process.exitCode = 1;
    }
} catch (error) {
    ...
    console.error(
        error instanceof Error && !("stdout" in error) && !("stderr" in error)
            ? error.message
            : "Git operation failed; secret scan incomplete.",
    );
    process.exitCode = 2;
} finally {
