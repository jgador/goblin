...
if (spawnSync("gitleaks", ["version"], { stdio: "ignore" }).status !== 0) {
    console.error(
        "Install Gitleaks 8.30.1 or newer first; see docs/secret-scanning.md.",
    );
    process.exit(1);
}
...
if (configured && configured !== ".githooks") {
    console.error(
        "An existing core.hooksPath is configured. Add the command from .githooks/pre-commit to your existing pre-commit hook instead.",
    );
    process.exit(1);
}
...
if (!configured) {
    ...
    if (
        ...
        console.error(
            "Existing Git hooks found. Add the command from .githooks/pre-commit to your existing pre-commit hook instead.",
        );
        process.exit(1);
    }
}
