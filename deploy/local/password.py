...
hash_password = runpy.run_path(str(REPO / "deploy/azure/hash-password.py"))[
    "hash_password"
]
...
def read_verifier(path):
    ...
    try:
        ...
        if (
            len(value) > 256
            or len(fields) != 4
            or fields[:2] != ["pbkdf2-sha256", "600000"]
        ):
            raise ValueError()
        if (
            len(base64.b64decode(fields[2], validate=True)) != 16
            or len(base64.b64decode(fields[3], validate=True)) != 32
        ):
            raise ValueError()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Choose a new password, replacing the saved verifier after confirmation",
    )
