...
def snapshot_source(repo, destination):
    ...
    with tarfile.open(destination, "w:gz") as archive:
        for name in sorted(
            set(os.fsdecode(entry) for entry in inventory.split(b"\0") if entry)
        ):
            relative = Path(name)
            ...
            if source.parent.resolve() != source.parent:
                raise RuntimeError(
                    f"Local source snapshots do not accept symlink directories: {name}"
                )
            if source.is_symlink():
                raise RuntimeError(
                    f"Local source snapshots do not accept symlinks: {name}"
                )
...
def owned_config():
    if not OWNER.exists():
        raise RuntimeError(
            "No direct local installation exists. Run npm run install:local -- start."
        )
...
def preflight(port):
    ...
    if (
        ...
        or "ID=ubuntu" not in Path("/etc/os-release").read_text()
    ):
        raise RuntimeError(
    ...
    for path in (
        ...
        Path("/var/lib/cni"),
        SYSTEMD / "k3s.service",
        SYSTEMD / "goblin-setup.service",
        SYSTEMD / "goblin-installer.service",
        SYSTEMD / "goblin-local.socket",
        SYSTEMD / "goblin-local.service",
        SYSTEMD / DATABASE_SOCKET,
        SYSTEMD / DATABASE_SERVICE,
    ):
        if path.exists():
            raise RuntimeError(
                f"{path} already exists outside this runner. Use a clean WSL distribution for this test."
            )
    if shutil.which("k3s"):
        raise RuntimeError(
    ...
    for candidate in (80, 443, selected_port):
        with socket.socket() as listener:
            ...
            try:
                ...
                raise RuntimeError(
                    f"WSL/Linux port {candidate} is already in use. Stop that listener first."
                ) from error
    return {
        "version": 2,
        "mode": "direct",
...
def configure_database(config, requested_port):
    ...
    if not active("k3s.service"):
        raise RuntimeError(
            "Start the local cluster first: npm run install:local -- start."
        )
    if "postgres_port" not in config and any(
        (SYSTEMD / unit).exists() for unit in (DATABASE_SOCKET, DATABASE_SERVICE)
    ):
        raise RuntimeError(
        ...
        for family, address in (
            ...
            with socket.socket(family) as listener:
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Local port {port} is occupied. Stop its existing port-forward or choose another port."
                    ) from error
    kubectl = ["k3s", "kubectl", "--kubeconfig=/etc/rancher/k3s/k3s.yaml"]
    run(
        [*kubectl, "get", "statefulset", "goblin-postgres", "-n", "goblin"],
    ...
    try:
        ...
        raise RuntimeError(
            "The local PostgreSQL service has no usable IPv4 cluster address."
        ) from error
    units = {
...
""",
    ...
    changed = any(
        not (SYSTEMD / name).exists() or (SYSTEMD / name).read_text() != text
    ...
    print(
        f"PostgreSQL is available at localhost:{port}; local access runs in the background. No kubectl port-forward is needed."
    )
...
def prepare_password():
    ...
    if not path.exists():
        ...
        if retained.exists():
            ...
            local_password.save_verifier(
                path,
                local_password.hash_password(legacy.read_text().removesuffix("\n")),
            )
...
def prepare(config):
    ...
    run(
        [
            "python3",
            REPO / "deploy/azure/build-setup-bundle.py",
            "--output-only",
            "--output",
            bundle,
        ]
    ...
    script = render_bootstrap(
        REPO / "deploy/azure", bundle.read_bytes(), "localhost", source_ref
    )
...
def start(args):
    ...
    complete_bootstrap = (
        bootstrapped.exists() and bootstrapped.read_text().strip() == "setup-ready"
    )
        ...
        if local_password.read_verifier(retained) != verifier:
            ...
            secret = run(
                [
                    ...
                    "-n",
                    "goblin",
                    f"--from-file=owner-password={password_file}",
                    "--dry-run=client",
                    "-o",
                    "json",
                ],
    ...
    for _ in range(30):
        ...
        raise RuntimeError(
            "The browser port is not ready. Inspect npm run install:local -- logs."
        )
    ...
    if state and state["status"] == "failed":
        print(
            "The previous attempt failed. Inspect logs, then run npm run install:local -- retry."
        )
...
def status(config):
    ...
    if config.get("postgres_port"):
        print(
            f"PostgreSQL: localhost:{config['postgres_port']} ({'listening' if active(DATABASE_SOCKET) else 'stopped'})"
        )
...
def reset():
    ...
    run(["systemctl", "daemon-reload"])
    print(
        "Local Goblin cluster and application data removed. The repository password verifier, Docker and download caches are retained."
    )
...
def main():
    ...
    create = commands.add_parser(
        "start", help="Show setup and install directly in WSL/Linux, or resume"
    )
    create.add_argument(
        "--http-port", type=int, choices=range(1024, 65536), metavar="PORT"
    )
    database = commands.add_parser(
        "database",
        help="Enable persistent local PostgreSQL access after database setup",
    )
    database.add_argument(
        "--port", type=int, choices=range(1024, 65536), metavar="PORT"
    )
    ...
    for name in ("reset", "destroy"):
        destroy = commands.add_parser(
            name, help="Remove this runner’s local cluster and application data"
        )
    ...
    if os.geteuid() != 0:
        os.execvp(
            ...
            [
                "sudo",
                "--preserve-env=GOBLIN_LOCAL_PASSWORD",
                sys.executable,
    ...
    if args.command in ("status", "logs"):
        ...
        if args.command == "status":
            ...
            paths = [
                path
                for path in (
                    Path("/var/log/goblin-bootstrap.log"),
                    Path("/var/log/goblin-installer.log"),
                )
                if path.exists()
            ]
            if not paths:
        ...
        if args.command == "start":
            ...
            if args.command == "database":
                ...
                if current["status"] == "ready":
                    raise RuntimeError(
                        "Installation is complete. Use reset --yes, then start for a fresh test."
                    )
                configure_forwarder(config)
                if (SYSTEMD / "k3s.service").exists():
                    run(["systemctl", "enable", "--now", "k3s"])
                ...
                print(
                    "Installer retry started; retained source and password are reused."
                )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Local installation: {error if not isinstance(error, subprocess.CalledProcessError) else 'Command failed; inspect npm run install:local -- logs and .goblin-local/bootstrap.log.'}",
            file=sys.stderr,
        )
        sys.exit(1)
