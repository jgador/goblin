...
def hash_password(password: str) -> str:
    ...
    if length > 128 or not password.strip() or any(
        ord(character) < 32 or ord(character) == 127 for character in password
    ):
        raise ValueError("Enter a non-blank Goblin password of up to 128 characters without control characters.")
    ...
    return "pbkdf2-sha256$600000$" + base64.b64encode(salt).decode("ascii") + "$" + base64.b64encode(digest).decode("ascii")
...
if __name__ == "__main__":
    try:
        ...
        sys.exit("Goblin password is invalid. Enter up to 128 characters, including a non-space character, without tabs or line breaks.")
