...
1. Click **Deploy to Azure**. On **Basics**, select your subscription, a dedicated
   resource group (suggested: `rg-goblin-prod`), and region.
   Keep **Company or software name** as `Goblin` or enter your own name.
2. Under VM credentials, choose **Generate new key pair** and name the key,
   or select an existing public key.
3. Leave **Advanced settings** at their defaults unless needed. Direct SSH is
   closed by default; enabling it requires an allowed source IP/CIDR.
4. On **Review + create**, select **Create**. For a new key, select **Download
   private key and create resource** and save the file for VM access later.
   Azure does not offer another download of the same private key.
5. Follow progress on the deployment page. After success, open **Outputs** for
   resource names, public IP, hostname, and administration commands.
...
SSD OS disk. Azure usage charges apply; check pricing, quota, and availability
in your region. Alternative VM sizes must support x64 Ubuntu and Trusted Launch.
