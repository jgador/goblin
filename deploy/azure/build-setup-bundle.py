...
mode.add_argument("--output-only", action="store_true", help="Build a local bundle without updating generated template inputs")
parser.add_argument("--output", type=Path, help="Also emit an executable .pyz for local tests or distribution")
...
outputs = {"setup-bundle.b64": base64.b64encode(data).decode() + "\n", "setup-bundle.sha256": hashlib.sha256(data).hexdigest() + "\n"}
for name, content in ({} if args.output_only else outputs).items():
    path = root / name
    if args.check:
        if not path.exists() or path.read_text() != content:
            raise SystemExit("Setup bundle is stale. Run python3 deploy/azure/build-setup-bundle.py")
    else:
...
print(f"Goblin setup bundle: {len(data)} bytes; sha256 {hashlib.sha256(data).hexdigest()}")
