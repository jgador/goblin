...
def private_write(path, contents):
    ...
    descriptor, temporary = tempfile.mkstemp(dir=path.parent, prefix=".goblin-", suffix=".tmp")
    try:
...
def connection(role, directory, host="localhost", port=5432):
    ...
    return (f'Host={host};Port={port};Database=goblin;Username=goblin_{role};SSL Mode=VerifyFull;GSS Encryption Mode=Disable;'
...
def main():
    ...
    parser.add_argument("--create-secret", choices=["admin"], help="Generate the image's initialization-only password Secret")
    parser.add_argument("--port", type=int, default=5432, help="Local database endpoint port for tooling")
    ...
    if args.create_secret:
        json.dump({"apiVersion": "v1", "kind": "Secret", "type": "Opaque",
                   ...
                   "stringData": {"password": secrets.token_hex(32)}}, sys.stdout)
        return
    ...
    for role in ("app", "admin"):
        for name in ("ca.crt", "tls.crt", "tls.key"):
            value = base64.b64decode(cluster_secrets[f"goblin-postgres-{role}-tls"][name], validate=True)
            if not value.strip():
    ...
    settings = {}
    for path, values in {
        web_path: {"Goblin": connection("app", Path("/etc/goblin-postgres"), host="goblin-postgres")},
        tooling_path: {"Goblin": connection("app", ROOT / ".goblin-postgres/app", port=args.port),
                       "GoblinAdmin": connection("admin", ROOT / ".goblin-postgres/admin", port=args.port)},
    }.items():
    ...
    for directory in (ROOT / ".goblin-postgres", ROOT / ".goblin-postgres/app", ROOT / ".goblin-postgres/admin"):
    ...
    print("Exported client certificates and updated password-free web/tooling appsettings.json files.")
...
if __name__ == "__main__":
    try:
        ...
        sys.exit("Cannot export PostgreSQL configuration. Check client Secrets and local file permissions.")
