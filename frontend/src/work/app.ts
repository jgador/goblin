...
function renderHome() {
    ...
    return `<section class="home"><div class="home-content"><div class="home-mark"><img class="home-portrait" src="/assets/branding/icon.svg" alt=""></div><p class="home-eyebrow">YOUR WORKSPACE</p><h1>What should we work on?</h1><p class="home-description">Turn an idea into an outcome, with Goblin alongside you.</p>${composer("new", "Describe the intended outcome…")}<div class="connection-nudge" ${available || !connectionsLoaded ? "hidden" : ""}><span>${disconnected ? "Connect an AI provider when you’re ready to start." : "Your AI connection may need attention."}</span><button data-action="settings">${disconnected ? "Connect an AI provider" : "Manage connections"}${icon("arrow")}</button></div></div></section>`;
}
