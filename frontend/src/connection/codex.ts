...
export async function mountCodex(root: HTMLElement) {
    ...
    function byId(
        id: string,
        kind: typeof HTMLElement = HTMLElement,
    ): HTMLElement {
        const element = root.querySelector(`#${id}`);
        if (!(element instanceof kind))
            throw new Error(`Missing or invalid page element: ${id}`);
        return element;
    }

    for (const container of root.querySelectorAll("[data-device-login-help]")) {
        container.append(
            byId(
                "device-login-help-template",
                HTMLTemplateElement,
        ...
        byId("connection-status").textContent =
            connectionCheck.status === "failed"
                ...
                : checking
                  ...
                  : "Connected";
        byId("connection-description").textContent =
            connectionCheck.status === "failed"
                ? connectionCheck.description
                : checking
                  ? `Checking that your ${accountName} is ready to use.`
                  : `Your ${accountName} is ready to use.`;
        byId("connection-mark").textContent = checking
            ? ""
            : status === "connected"
              ? "✓"
              : "!";
        byId("check-connection-button").textContent = checking
            ? "Checking connection…"
            : status === "failed"
              ? "Retry check"
              : "Check again";
    ...
    function connectionFailure(error: unknown): ConnectionCheck {
        ...
        switch (error instanceof ApiError ? error.code : "") {
            case "prompt_unauthorized":
                return {
                    status: "failed",
                    title: chatgpt ? "Sign-in expired" : "API key rejected",
                    description: chatgpt
                        ? "Disconnect Codex and sign in again to reconnect your ChatGPT account."
            ...
            case "prompt_limit_reached":
                return {
                    ...
                    description: chatgpt
                        ? "Your account has reached a usage or rate limit. Try again later or check your ChatGPT plan."
            ...
            case "prompt_access_denied":
                return {
                    ...
                    description:
                        "This account cannot use the selected model. Check your account’s access and permissions.",
                };
            case "prompt_timeout":
                return {
                    status: "failed",
                    ...
                    description:
                        "The check didn’t finish. Try again in a moment.",
                };
            case "prompt_cancelled":
                return {
                    status: "failed",
                    title: "Connection check cancelled",
                    description:
                        "The check was interrupted. Try again when you’re ready.",
                };
            case "not_connected":
                return {
                    status: "failed",
            ...
            case "prompt_in_progress":
                return {
                    status: "failed",
                    title: "Another check is running",
                    description:
                        "Wait for the current check to finish, then try again.",
                };
            default:
                return {
                    status: "failed",
                    ...
                    description:
                        "We couldn’t verify access. Check your connection and try again.",
                };
        }
    }
    ...
    const accountEmail = (account: Account | null | undefined) =>
        account?.type === "chatgpt" ? account.email : null;

    function render(next: AuthenticationState) {
        if (
            !next.account ||
            next.account.type !== state?.account?.type ||
            accountEmail(next.account) !== accountEmail(state?.account)
        ) {
            connectionCheck = { status: "unchecked" };
        }
        state = next;
        ...
        if (account) {
            ...
            byId("verification-link", HTMLAnchorElement).href =
                next.login.verificationUrl;
            showPanel("pending");
        } else {
    ...
    async function api<Path extends keyof ApiResponses>(
        path: Path,
                ...
                if (requestVersion === viewVersion && unlocked) {
                    ...
                    if (state?.account) {
                        connectionCheck = {
                            ...
                            description:
                                "Goblin couldn’t refresh your account status. Check your connection and try again.",
                        };
                        renderConnectionCheck();
                    } else
                        message(
    ...
    byId("chatgpt-button").addEventListener("click", () =>
        action("Getting a sign-in code…", async () => {
    ...
    byId("copy-code").addEventListener("click", async () => {
        try {
            ...
            message(
                "Select and copy the code above, then paste it on OpenAI’s sign-in page.",
                "info",
            );
        }
    });
