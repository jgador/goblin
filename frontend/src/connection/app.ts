...
function showPanel(name: Panel) {
    for (const item of ["loading", "locked", "connect", "pending", "connected"])
...
async function checkConnection() {
    ...
    await action("Verifying connection…", async () => {
        ...
        try {
            ...
            if (requestVersion !== viewVersion || !unlocked || !state?.account)
                return;
...
async function api<Path extends keyof ApiResponses>(
    ...
    if (!response.ok) {
        ...
        throw new ApiError(
            ...
            failure?.error?.message || "Something went wrong. Please retry.",
        );
    }
